...
export class TransactionsComponent implements OnInit {
  ...
  cancelledTransactionsDataSource = new MatTableDataSource<Transaction>();
  ...
  loadTransactions(): void {
    ...
    this.transactionService
      .getTransactionsByClientId(storedClient.id)
      .subscribe(
        (transactions) => {
          ...
          this.dataSource.data = activeTransactions;

          const cancelledTransactions = transactions
            .filter((transaction) => transaction.transactionType === 'canceled')
            .map((transaction, index) => ({
              ...transaction,
              position: index + 1,
            }));
          this.cancelledTransactionsDataSource.data = cancelledTransactions;
        },
        (error) => {
          Swal.fire({
            icon: 'error',
